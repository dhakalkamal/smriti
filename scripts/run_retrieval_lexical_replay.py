...
def main(argv: Sequence[str] | None = None) -> int:
    """Run an offline Stage 13c-1 lexical replay from diagnostic output."""

    parser = argparse.ArgumentParser(
        description="Replay Stage 12 diagnostic candidates against eval-only lexical profiles.",
    )
    parser.add_argument(
        "--input-run",
        type=Path,
    ...
    try:
        ...
        replay_payload = run_stage13_lexical_replay(
    ...
    if not isinstance(replay_metadata, dict):
        raise ValueError("lexical replay payload metadata must be an object")
    input_run_id = str(replay_metadata["input_run_id"])
    run_dir = args.output_dir / input_run_id
    report_path = args.report_dir / f"{input_run_id}-lexical-replay.md"
    run_dir.mkdir(parents=True, exist_ok=True)
    args.report_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "lexical_replay.json").write_text(
        json.dumps(replay_payload, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    report_path.write_text(
        stage13_lexical_replay_report_to_markdown(replay_payload),
        encoding="utf-8",
    )

    print(f"Wrote Stage 13c-1 lexical replay output to {run_dir / 'lexical_replay.json'}")
    print(f"Wrote Stage 13c-1 Markdown report to {report_path}")
    return 0
