...
def main(argv: Sequence[str] | None = None) -> int:
    """Run an offline Stage 12b-2 weight sweep from diagnostic output."""

    parser = argparse.ArgumentParser(
        description="Replay Stage 12 diagnostic candidates against fixed weight profiles.",
    )
    parser.add_argument(
        "--input-run",
        type=Path,
    ...
    from smriti.memory.eval import (
        ...
        stage12_weight_sweep_report_to_markdown,
    )

    run_json_path = args.input_run / "run.json"
    cases_jsonl_path = args.input_run / "cases.jsonl"
    run_payload = _load_json_object(run_json_path)
    metadata = _metadata_from_run_payload(run_payload)
    try:
        cases = tuple(
            stage12_case_result_from_dict(record)
            for record in _load_jsonl_objects(cases_jsonl_path)
        )
    except InvalidRetrievalRequestError as exc:
        parser.error(str(exc))

    sweep_payload = run_stage12_weight_sweep(
    ...
    sweep_metadata = sweep_payload["metadata"]
    if not isinstance(sweep_metadata, dict):
        raise ValueError("weight sweep payload metadata must be an object")
    input_run_id = str(sweep_metadata["input_run_id"])
    run_dir = args.output_dir / input_run_id
    report_path = args.report_dir / f"{input_run_id}-weight-sweep.md"
    run_dir.mkdir(parents=True, exist_ok=True)
    args.report_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "weight_sweep.json").write_text(
        json.dumps(sweep_payload, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    report_path.write_text(
        stage12_weight_sweep_report_to_markdown(sweep_payload),
        encoding="utf-8",
    )

    print(f"Wrote Stage 12b-2 weight sweep output to {run_dir / 'weight_sweep.json'}")
    print(f"Wrote Stage 12b-2 Markdown report to {report_path}")
    return 0
